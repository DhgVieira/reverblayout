 ...
 *
 * @name Reverb_Controller_Action
 * @see Zend_Controller_Action
 */
class Reverb_Controller_Action extends Zend_Controller_Action {
	/**
	...
	protected function listAction() {
		...
		try {
			...
			$paginator->setPageRange($config->reverb->paginator->range);
		...
		if($this->_request->getParam("nopage", 0) == 1) {
			...
			$paginator->setItemCountPerPage($config->reverb->paginator->perpage);
		}
...
//			$uploader->isValid();
//			$destination = APPLICATION_PATH . "/../arquivos/uploads/destaques";
...
//					die();
				...
				if($referer_url != "") {
					// Busca o basepath
					$config = Zend_Registry::get('config');
					$basepath = $config->Reverb->config->basepath;
