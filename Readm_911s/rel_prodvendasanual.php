...
<table width="820" class="font12">
    ...
    $sql = "
    SELECT
				    DS_CATEGORIA_PTRC, sum(NR_QTDE_CESO) as total, 
    ...
    WHERE 
    				...
    				DT_COMPRA_COSO between STR_TO_DATE('13/05/2010 00:00:00','%d/%m/%Y %H:%i:%s') 
    				and STR_TO_DATE('13/08/2010 23:59:59','%d/%m/%Y %H:%i:%s') 
    				AND NR_SEQ_LOJAS_PRRC = 1 
    				AND NR_SEQ_TIPO_PRRC = 4 
    				AND NR_SEQ_CADASTRO_COSO NOT IN (8074, 6605, 22364)
    GROUP BY 
    				NR_SEQ_TIPO_PRRC order by total desc
    ";
	...
	$sql = "SELECT NR_SEQ_PRODUTO_CESO, DS_CATEGORIA_PTRC, DS_REFERENCIA_PRRC, DS_PRODUTO2_PRRC, DS_LOJA_LJRC, sum(NR_QTDE_CESO) as total, 
			...
			and DT_COMPRA_COSO between STR_TO_DATE('$dataini','%d/%m/%Y %H:%i:%s') and STR_TO_DATE('$datafim','%d/%m/%Y %H:%i:%s') AND
	...
	$sql .= " GROUP BY NR_SEQ_PRODUTO_CESO order by total desc";
