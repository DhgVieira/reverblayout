...
<table width="<?php echo $largura; ?>" >
	<tr>
        ...
        <td class="font16" align="center" height="70"><strong>Relat&oacute;rio de Atividade no Site</strong><br /><font size="-1">Referente: <?php echo $mes?>/<?php echo $ano?></font></td>
...
for ($f=0;$f<=11;$f++){
    ...
    $sql = "SELECT count(*) from topicos where NR_SEQ_CADASTRO_TOSO = ".$moderador[$f]." and DT_CADASTRO_TOSO BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    ...
    $sql = "select count(*) from msgs where NR_SEQ_CADASTRO_MESO = ".$moderador[$f]." and DT_CADASTRO_MESO BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "select count(*) from enquetes where NR_SEQ_CADASTRO_EQRC = ".$moderador[$f]." and DT_CRIACAO_EQRC BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "SELECT count(*) from enquetes_coments where NR_SEQ_CADASTRO_CERC = ".$moderador[$f]." and DT_COMENT_CERC BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "select count(*) from blog_coments where NR_SEQ_CADASTRO_CBRC = ".$moderador[$f]." and DT_CADASTRO_CBRC BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "select count(*) from produtos_coments where NR_SEQ_CADASTRO_PCRC = ".$moderador[$f]." and DT_COMENT_PCRC BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "select count(*) from me_fotos_coments where NR_SEQ_CADASTRO_MCRC = ".$moderador[$f]." and DT_CADASTRO_MCRC BETWEEN '$dtaini' and '$dtafim'";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
    ...
    $sql = "SELECT count(*) from compras where NR_SEQ_CADASTRO_COSO = ".$moderador[$f]." and ST_COMPRA_COSO <> 'C' and DT_COMPRA_COSO BETWEEN '$dtaini' and '$dtafim';";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
         $row = mysql_fetch_row($st);
         $total += $row[0];
