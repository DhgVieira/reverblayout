...
<table width="<?php echo $largura; ?>" >
	<tr>
        ...
        <td class="font16" align="center" height="70"><strong>Relat&oacute;rio de Vendas de Atacado M&ecirc;s <?php echo date("m",strtotime($primrodiacomp));?></strong><br /><font size="-1">Loja: Londrina/Internet</font></td>
...
<table width="<?php echo $largura; ?>" >
	<tr>
    	<td height="70" align="left" class="fontlogo" width="200"><strong>ReverbCity</strong></td>
        <td class="font16" align="center" height="70"><strong>Relat&oacute;rio de Vendas de Atacado SP M&ecirc;s <?php echo date("m",strtotime($primrodiacomp));?></strong><br /><font size="-1">Loja: Pocket Store SP</font></td>
        <td class="font12" align="right" height="70" width="200"><?php echo date("d/m/Y G:i"); ?></td>
    </tr>
    <tr><td colspan="3" height="1"><img src="img/xb.gif" height="2" width="<?php echo $largura; ?>" /></td></tr>
</table>
...
<script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Mes/Ano', 'Vlr Total'],
          <?php
            $str = "select year(DT_COMPRA_COSO), month(DT_COMPRA_COSO), sum(VL_TOTAL_COSO)
                            from compras, cadastros where
                            	NR_SEQ_CADASTRO_COSO = NR_SEQ_CADASTRO_CASO AND 
                            	TP_CADASTRO_CACH = 1 and NR_SEQ_CADASTRO_COSO NOT IN (8074, 6605, 22364) AND
                            	ST_COMPRA_COSO <> 'C' and ST_COMPRA_COSO <> 'A'
                     GROUP BY  year(DT_COMPRA_COSO), month(DT_COMPRA_COSO)
                    order by DT_COMPRA_COSO desc limit 12";
            $st = mysql_query($str);
            if (mysql_num_rows($st) > 0) {
                while($row = mysql_fetch_row($st)) {
          ?>
