...
<title>Vendas Promo inicio 13/02</title>
                        ...
                        <div id="listaprodutos" style="width: 990px;">
                            ...
                            $primeiro_registro = ($pagina*$num_por_pagina) - $num_por_pagina;
                             ...
                             $sql = "SELECT NR_SEQ_PRODUTO_PRRC, VL_PRODUTO_PRRC, DS_PRODUTO2_PRRC, DS_EXT_PRRC, TP_DESTAQUE_PRRC, 
                                    ...
                                    AND DT_COMPRA_COSO > '2014-02-13 11:00:00'
                                    AND NR_SEQ_PRODUTO_PRRC <> 5230
                            ...
                            if (mysql_num_rows($st) > 0) {
                                ...
                                while($row = mysql_fetch_row($st)) {
                                    ...
                                    ?>
                                    <div class="produto"  style="margin: 0 0 7px 6px;position: relative; z-index:1; height: 370px;width: 160px;text-align: center;">
                                    <div style="text-align: center;"><strong><?php echo $qtde_estoq ?></strong></div>
                                      <?php if ($ds_ext == "swf") {?>
                                      <object data="../images/produtos/<?php echo $id_prod; ?>.<?php echo $ds_ext; ?>" type="application/x-shockwave-flash" width="180" height="210">
                                        <param name="quality" value="high" />
                                        <param name="flashvars" value="URLname=<?php echo $id_prod; ?>" />
                                        <param name="movie" value="../images/produtos/<?php echo $id_prod; ?>.<?php echo $ds_ext; ?>" />
                                      ...
                                      ?>
                                      <a href="/produto/<?php echo $ds_categoria ?>/<?php echo urlencode($ds_prod_url); ?>"><img src="../images/produtos/<?php echo $id_prod; ?>.<?php echo $ds_ext; ?>" border="0" alt="<?php echo $ds_prod; ?>" width="120" /></a>
                                      <?php } ?>
...
$sql_efetivo = "SELECT 
                    ...
                    COUNT(NR_SEQ_COMPRA_COSO) as quantidade,
                    SUM(VL_FRETE_COSO) as vl_frete
                    FROM
        ...
        if (mysql_num_rows($st_efetivo) > 0) {
        	$rowefetivo = mysql_fetch_row($st_efetivo);
	            ...
	            $total_pago = $rowefetivo[1];
                $frete_pago = $rowefetivo[2];
                $pago_sem_frete = $valor_pago - $frete_pago;
	    }
...
$sql_aberto = "SELECT 
        ...
        if (mysql_num_rows($st_aberto) > 0) {
        	$row = mysql_fetch_row($st_aberto);
	           
	            $valor = $row[0];
	            $total = $row[1];
                $frete = $row[2];
                $aberto_sem_frete = $valor - $frete;
	    }




$sql_cancelado = "SELECT 
                      SUM(VL_TOTAL_COSO) as total,
                        COUNT(NR_SEQ_COMPRA_COSO) as quantidade, 
                        SUM(VL_FRETE_COSO) as vl_frete
                    FROM
        ...
        $st_cancelado = mysql_query($sql_cancelado);

        if (mysql_num_rows($st_cancelado) > 0) {
        	$row_c = mysql_fetch_row($st_cancelado);
	            $valor_c = $row_c[0];
	            $total_c = $row_c[1];
                $frete_c = $row_c[2];
                $cancel_sem_frete = $valor_c - $frete_c;
	        }
...
?>
        ...
        <?php if($qtde_total <= 0){
          ...
          <b>Quantidade Total Pago</b>: <?php echo $total_pago ?><br />
      	  <b>Valor Total Pago</b>: R$<?php echo number_format($valor_pago, 2, ",", ".")?><br />
          <b>Valor Total Pago sem frete</b>: R$<?php echo number_format($pago_sem_frete, 2, ",", ".")?><br />
      	  <b>Quantidade Total Aberto</b>: <?php echo $total ?><br />
      	  <b>Valor Total Aberto</b>: R$<?php echo number_format($valor, 2, ",", ".")?><br />
          <b>Valor Total Aberto sem frete</b>: R$<?php echo number_format($aberto_sem_frete, 2, ",", ".")?><br />
      	  <b>Quantidade Total Cancelados</b>: <?php echo $total_c ?><br />
      	  <b>Valor Total Cancelado</b>: R$<?php echo number_format($valor_c, 2, ",", ".")?><br />
          <b>Valor Total Cancelado sem frete</b>: R$<?php echo number_format($cancel_sem_frete, 2, ",", ".")?><br />
