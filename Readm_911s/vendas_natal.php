...
?>
    <div id="geral" style="background-color: white;">
        ...
        <h3>Vendas de natal - 06/12/2013 até o presente momento.</h3>
                        ...
                        <div id="listaprodutos" style="width: 990px;">
                            ...
                            $primeiro_registro = ($pagina*$num_por_pagina) - $num_por_pagina;
                             ...
                             $sql = "SELECT NR_SEQ_PRODUTO_PRRC, VL_PRODUTO_PRRC, DS_PRODUTO2_PRRC, DS_EXT_PRRC, TP_DESTAQUE_PRRC, 
                                     ...
                                     NR_SEQ_CADASTRO_COSO not in (8074, 22364) and NR_SEQ_LOJA_COSO = 1 AND
                                    ST_COMPRA_COSO <> 'C' and ST_COMPRA_COSO <> 'A'  and
                                    ((DT_COMPRA_COSO > '2013-12-06'))
                      ...
                      if ($tipo != "4") { ?>
                        ...
                        for ($f=27;$f<=32;$f++){
                            $sqltam = "SELECT NR_SEQ_PRODUTO_ESRC, NR_SEQ_ESTOQUE_ESRC, DS_SIGLA_TARC, NR_QTDE_ESRC, NR_SEQ_PRODUTO_PRRC, NR_SEQ_CATEGORIA_PRRC FROM estoque, tamanhos, produtos
                                        WHERE NR_SEQ_TAMANHO_ESRC = NR_SEQ_TAMANHO_TARC AND NR_SEQ_LOJA_ESRC = 1 AND NR_SEQ_PRODUTO_ESRC = $id_prod AND NR_SEQ_CATEGORIA_PRRC = 185
                                        AND NR_SEQ_TAMANHO_ESRC = $f";
                            $sttam = mysql_query($sqltam);
                       ...
                       <ul style="clear: both; z-index: 8; margin: 0; padding: 0; width: 160px; text-align: center;">
                        ...
                        $sqltam = "SELECT NR_SEQ_PRODUTO_ESRC, NR_SEQ_ESTOQUE_ESRC, DS_SIGLA_TARC, NR_QTDE_ESRC, NR_SEQ_PRODUTO_PRRC, NR_SEQ_CATEGORIA_PRRC FROM estoque, tamanhos, produtos
                                    WHERE NR_SEQ_TAMANHO_ESRC = NR_SEQ_TAMANHO_TARC AND NR_SEQ_LOJA_ESRC = 1 AND NR_QTDE_ESRC > 0 AND NR_SEQ_PRODUTO_ESRC = $id_prod AND NR_SEQ_CATEGORIA_PRRC = 185
        ...
        <?php $sql_total = "SELECT 
                            ...
                            FROM
                                reverbcity22.compras
                            WHERE
                                NR_SEQ_CADASTRO_COSO not in (8074 , 22364)
                                    and NR_SEQ_LOJA_COSO = 1
                                 
                                    and ((DT_COMPRA_COSO > '2013-12-06'));";
       ...
       $sql_pagos = "SELECT 
        ...
        $sql_promo = "SELECT 
                            ...
                            WHERE
                                NR_SEQ_LOJA_COSO = 1
                                    AND VL_TOTAL_COSO > 150.0
                                    AND ST_COMPRA_COSO = 'V'
                                    and ((DT_COMPRA_COSO > '2013-12-06'));";

        $st_promo = mysql_query($sql_promo);

        if (mysql_num_rows($st_promo) > 0) {
            $rowpromo= mysql_fetch_row($st_promo);
            
            $valor_promo = $rowpromo[0];
        }   


         $sql_promo_pago = "SELECT 
                                SUM(VL_TOTAL_COSO) as total
                            FROM
                                reverbcity22.compras
                            WHERE
        ...
        $st_promo_pago = mysql_query($sql_promo_pago);

        if (mysql_num_rows($st_promo_pago) > 0) {
            $rowpromo_pago= mysql_fetch_row($st_promo_pago);
            
            $valor_promo_pago = $rowpromo_pago[0];
        }       



        $sql_promo_peca = "SELECT 
        ...
        $st_promo_pecas = mysql_query($sql_promo_peca);

        if (mysql_num_rows($st_promo_pecas) > 0) {
            $rowpromo_peca= mysql_fetch_row($st_promo_pecas);
            
            $total_promo = $rowpromo_peca[0];
        }  

        $sql_promo_peca_pago = "SELECT 
                                COUNT(NR_SEQ_COMPRA_COSO) as total
                            FROM
                                reverbcity22.compras
                            WHERE
        ...
        $st_promo_pecas_pago = mysql_query($sql_promo_peca_pago);

        if (mysql_num_rows($st_promo_pecas_pago) > 0) {
            $rowpromo_peca_pago= mysql_fetch_row($st_promo_pecas_pago);
            
            $total_promo_pago = $rowpromo_peca_pago[0];
        }  


        $valor_total_promo = $valor_promo + $valor_promo_pago;
        $total_pecas = $total_promo_pago + $total_promo;
        $media_compra = $valor_total_promo / $total_pecas;   
?>
        ...
        <center> <hr color="black" width="300px" height="3px"></center>
         ...
         <b> Valor Total Promo (confirmados e entregues):</b> R$ <?php echo number_format($valor_total_promo, 2, ",", ".")?></br><br />
         <b> Total de compras com promo (Acima de R$150):</b>  <?php echo $total_pecas?></br><br />
         <b> Valor médio de pedidos da promo:</b> R$ <?php echo number_format($media_compra, 2, ",", ".")?></br><br />
         <center> <hr color="black" width="300px" height="3px"></center>


        <h3>Camisetas que entraram como gratuítas</h3>
        <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
                <td valign="top">
                    <div id="shop" style="background-color: white;">
                        ...
                        <div id="listaprodutos" style="width: 990px;">
                            ...
                            $primeiro_registro = ($pagina*$num_por_pagina) - $num_por_pagina;
                             ...
                             $sql = "SELECT NR_SEQ_PRODUTO_PRRC, VL_PRODUTO_PRRC, DS_PRODUTO2_PRRC, DS_EXT_PRRC, TP_DESTAQUE_PRRC, 
                                     DS_FRETEGRATIS_PRRC, VL_PROMO_PRRC, DS_CATEGORIA_PTRC, SUM(NR_QTDE_CESO) as total, VL_PRODUTO_CESO 
                                     from compras, cestas, produtos, produtos_tipo where 
                                     NR_SEQ_COMPRA_COSO = NR_SEQ_COMPRA_CESO AND
                                     NR_SEQ_PRODUTO_CESO = NR_SEQ_PRODUTO_PRRC AND
                                     NR_SEQ_TIPO_PRRC = NR_SEQ_CATEGPRO_PTRC AND                                    
                                     NR_SEQ_CADASTRO_COSO not in (8074, 22364) and NR_SEQ_LOJA_COSO = 1 AND VL_PRODUTO_CESO = 0 AND
                                    ST_COMPRA_COSO <> 'C' and ST_COMPRA_COSO <> 'A'  and
                                    ((DT_COMPRA_COSO > '2013-12-06'))
      ...
      <hr bgcolor="black" height="15px" width="100%">
        </br>
         <?php if($qtde_total <= 0){
                    echo "Não foram efetuadas vendas até o momento";
                }else{ ?>
                  Quantidade Total de camisetas gratuítas: <?php echo $qtde_total ?><br />
