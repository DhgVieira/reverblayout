...
<input type="hidden" name="assunto" value="Relatorio de Novos Cadastros" />
...
<table width="820" >
	...
	<tr>
        ...
        <td class="font16" align="center" height="70"><strong>Relatório de Novos Cadastros</strong><br /><font size="-1"><?php echo $mes.' - '.$ano;?></font></td>
...
<table >
	<tr>
    	<td height=70 align=left class=fontlogo width=200><strong>ReverbCity</strong></td>
        <td class=font16 align=center height=70><strong>Relatório de Novos Cadastros</strong><br /><font size=-1>".$mes.'/'.$ano."</font></td>
...
";
    ...
    $sql = "select count(*) from cadastros WHERE MONTH(DT_CADASTRO_CASO) = $mes and YEAR(DT_CADASTRO_CASO) = $ano AND DS_SEXO_CACH in ('F','Feminino')";
    $st = mysql_query($sql);
    ...
    $total_masc = 0;
    $sql = "select count(*) from cadastros WHERE MONTH(DT_CADASTRO_CASO) = $mes and YEAR(DT_CADASTRO_CASO) = $ano AND DS_SEXO_CACH in ('M','Masculino')";
    $st = mysql_query($sql);
    if (mysql_num_rows($st) > 0) {
        $row = mysql_fetch_row($st);
        $total_masc = $row[0];
