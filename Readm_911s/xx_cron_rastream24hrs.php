				...
				$stnf = mysql_query($sqlnf);
					if (mysql_num_rows($stnf) > 0) {
						...
						if ($stcompra == 'A'){
							...
							$subject  = utf8_encode("ReverbCity - Confirma��o de Envio (Rastreamento)!");
...
if (($totalerro > 0) || ($total > 0)){
	...
	$corpo = IncluiPapelCarta("sistema",$texto,"IMPORTAÇÃO DOS C�DIGOS DE RASTREAMENTO");

	EnviaMailer("atendimento@reverbcity.com","Reverbcity","contato@reverbcity.com","Tony","","Importação dos códigos de rastreamento processada!",$corpo);
	EnviaMailer("atendimento@reverbcity.com","Reverbcity","atendimento@reverbcity.com","Miria","","Importação dos códigos de rastreamento processada!",$corpo);
}else{
	$corpo = IncluiPapelCarta("sistema",'Rastreamento rodou mas não encontrou nada',"IMPORTAÇÃO DOS CÓDIGOS DE RASTREAMENTO");
	EnviaMailer("atendimento@reverbcity.com","Reverbcity","desenvolvimento@reverbcity.com","DEV - RVB","","Importação dos códigos de rastreamento processada!",$corpo);
}
mysql_close($con);
?>