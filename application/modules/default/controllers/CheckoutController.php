...
class CheckoutController extends Zend_Controller_Action {
    ...
    public function adicionarcarrinhoAction() {
        ...
        $valor_promo_xgl = $produto['VL_PROMO_XGL_PRRC'];
        ...
        if ($sigla == 'XGL') {
            if ($valor_promo_xgl > 0) {
                $valor_promo = $valor_promo_xgl;
            }
        }
        ...
        if ($this->_request->isPost()) {
            ...
            foreach ($quantidades as $key => $quantidade) {
                ...
                if ($quantidade > 0 and $quantidade != "") {
                    ...
                    if ($produto->NR_QTDE_ESRC <= 0) {
                        $messages->error = "Produto fora de estoque.";
                        $this->_redirect($_SERVER['HTTP_REFERER']);
                    }

                    if ($produto->NR_QTDE_ESRC < $quantidade) {
                        $messages->error = "Quantidade indisponivel no estoque.";
                        $this->_redirect($_SERVER['HTTP_REFERER']);
                    }
...
//								</tr>";
                ...
                $mensagem_baixo .= "<tr border=\"0\" cellpadding=\"0\" cellspacing=\"0\">
                                                    <td colspan=\"15\" bgcolor=\"#FFFFFF\">
                                                            <a href='https://www.reverbcity.com/segunda-via-boleto/{$idpedido}' style=\"text-decoration:none; font-family:Verdana; font-size:12px; text-align: center; color: #FFFFFF; background-color: #e85238; color: #FFF; width: 150px; height: 50px; border: none; float:right; padding: 15px; line-height: 50px;\">
                                                             IMPRIMIR BOLETO 
                                                             </a>
                                                     </td>
                            </tr>";
...
//			</soapenv:Envelope>
            ...
            /* /*			$dados = '
              ...
              <soap:Body>
              <CalcPrecoPrazo xmlns="http://ws.correios.com.br">
              <nCdEmpresa>'. $nCdEmpresa .'</nCdEmpresa>
              <sDsSenha>'. $sDsSenha .'</sDsSenha>
              <nCdServico>'. $sCepOrigem .'</nCdServico>
              ...
              <nVlComprimento>'. $nVlPeso .'</nVlComprimento>
...
//			$retorno = $resp['soap:Envelope']['soap:Body']['CalcPrecoPrazoResponse']['CalcPrecoPrazoResult']['Servicos']['cServico'];
            ...
            $valor_correio = str_replace(",", ".", $total);
            ...
            if ($peso_lista->DS_FRETEGRATIS_PRRC == 'S') {
                $retorno['Valor'] = 0;
...
//			$retorno = $resp['soap:Envelope']['soap:Body']['CalcPrecoPrazoResponse']['CalcPrecoPrazoResult']['Servicos']['cServico'];
            ...
            echo "<pre id='retornocorreio' style='display: none'>";
            print_r($retorno['Resposta']);
            echo "</pre>"/
...
//			}
            ...
            if ($sessao_promo->brinde == 0 || $sessao_promo->sale == 0) {
