<section class="products">
    <h1 class="rvb-title">Reverb <span>Converse</span></h1>
...
<div class="rvb-column left">
        ...
        <ul class="pagination">
            {if $pages.previous}
                <li class="item">
                    <a title="Página Anterior" href="{$this->url(["module"=>"default", "controller"=>"loja", "action"=>"converse", "page"=>{$pages.previous}], "converse", TRUE)}">◄</a>
                </li>
            {/if}
           {section name=page_loop start=$this->contadores->current_page-1 loop=$this->contadores->current_page+3 step=1}
                {if $smarty.section.page_loop.index+1 == $this->contadores->current_page}
                    <li class="item">
                        <a href="{$this->url(["module"=>"default","controller"=>"loja",  "action"=>"converse", "page"=>$smarty.section.page_loop.index+1], 'converse', TRUE)}" class="active">
                            {$smarty.section.page_loop.index+1}
                        </a>
                    </li>
                {else}
                    <li class="item">
                        <a href="{$this->url(["module"=>"default","controller"=>"loja",  "action"=>"converse", "page"=>$smarty.section.page_loop.index+1], 'converse', TRUE)}">
                            {$smarty.section.page_loop.index+1}
                        </a>
                    </li>
                {/if}
            ...
            {if $pages.next}
                <li class="item">
                    <a title="Página Anterior" href="{$this->url(["module"=>"default", "controller"=>"loja", "action"=>"converse", "page"=>{$pages.next}], "converse", TRUE)}">►</a>
                </li>
            {/if}
        </ul>
    </div>
