...
<div class="full-title">
    ...
    <a href="{$this->url(["topico"=>{$this->createslug($topico->DS_TOPICO_TOSO)}, "idforum"=>{$topico->NR_SEQ_TOPICO_TOSO}], "denunciarforum", TRUE)}"
       class="rvb-forum-button report">Denunciar irregularidades</a>
</div>
{*
...
<div class="rvb-forum-details clearfix">
    {if $_logado neq 1}
        <p class="not-logado">
            Olá! Você precisa estar logado para comentar. <a href="{$this->url([], "reverbme", TRUE)}">Clique aqui </a>
            e faça um cadastro super rápido!
        </p>
    {else}
...
<div id="detalheforum-comments">
            ...
            <div class="list-of-comments clearfix">
                ...
                <div class="comment-item">
                    ...
                    <div class="comment-person">
                        <a href="{$this->url(["nome"=>{$this->createslug($msg['DS_NOME_CASO'])}, "idperfil"=>{$msg['NR_SEQ_CADASTRO_CASO']}], "perfil", TRUE)}"
                           class="image thumb-user">
                            {if file_exists("arquivos/uploads/reverbme/$foto_completa")}
                                <img src="{$this->Url(['tipo'=>"reverbme", 'crop'=>1, 'largura'=>55, 'altura'=>58, 'imagem'=>$foto_completa],"imagem", TRUE)}"
                                     alt="{$msg['DS_NOME_CASO']}"/>
                            {else}
                                <img src="{$this->Url(['tipo'=>"error", 'crop'=>1, 'largura'=>55, 'altura'=>58, 'imagem'=>'not_found_bkp.jpg'],"imagem", TRUE)}"
                                     alt="{$msg['DS_NOME_CASO']}"/>
                            {/if}
                        </a>
                    </div>

                    <p class="comment-user left">
                        <a href="{$this->url(["nome"=>{$this->createslug($msg['DS_NOME_CASO'])}, "idperfil"=>$msg['NR_SEQ_CADASTRO_CASO']], "perfil", TRUE)}"
                           class="image">{$msg['DS_NOME_CASO']}</a>
                        {if $codigo_usuario eq $msg['NR_SEQ_CADASTRO_CASO'] or $codigo_usuario == 2}
                            <a href="{$this->url(["idcomentario"=>{$msg['NR_SEQ_MSG_MESO']}], "deletarcomentarioforum", TRUE)}"
                               class="md-close">Excluir</a>
                        {/if}
                    <ul class="status-post right">
                        ...
                        <li class="status-item">
                            <a href="{$this->url(["idmsg"=>{$msg['NR_SEQ_MSG_MESO']}], 'naocurtirmsg', TRUE)}"><span
                                        class="notlikes"> {$msg['NR_NAOCURTIRAM_MESO']} não curtiram</span></a>
                        </li>*}
                        <li class="status-item hide">
                            <span class="answers">{$msg->findDependentRowset('Default_Model_Mensagens')->count()}</span> Respostas
                        </li>
                        <li class="status-item">
                            <span class="reply reply-comment-btn">Responder</span>
                        </li>
                        <li class="status-item clock">
                    <span class="timestamp">
                        <abbr class="timeago"
                              title="{$msg['DT_CADASTRO_MESO']|date_format:"%B %e, %Y"}">{$msg['DT_CADASTRO_MESO']|date_format:"%B %e, %Y"}</abbr> atrás
                    </span>
                        </li>
                    </ul>
                    </p>
                    ...
                    <div class="comment-detail">
                        ...
                        {foreach from=$msg->findDependentRowset('Default_Model_Mensagens') item=mensagem_filhas}
                            <div class="replied-item">
                                {if $codigo_usuario eq $mensagem_filhas->NR_SEQ_CADASTRO_CASO or $codigo_usuario == 2}
                                    <a href="{$this->url(["idcomentario"=>{$mensagem_filhas->NR_SEQ_MSG_MESO}], "deletarcomentarioforum", TRUE)}"
                                       class="md-close">Excluir</a>
                                {/if}
                                ...
                                <ul class="status-comment">
                                    <li class="status-item clock">
                                        <abbr class="timeago"
                                              title="{$mensagem_filhas->DT_CADASTRO_MESO|date_format:"%B %e, %Y"}">{$mensagem_filhas->DT_CADASTRO_MESO|date_format:"%B %e, %Y"}</abbr>
                                        atrás
                                        </span>
                                    </li>
                                </ul>
                        ...
                        <div class="user-reply-comment disabled">
                            ...
                            <form action="{$this->url(["idforum"=>{$topico->NR_SEQ_TOPICO_TOSO}], "comentarforum", TRUE)}"
                                  method="post">
                                <input type="hidden" name="idmensagem_pai" value="{$msg['NR_SEQ_MSG_MESO']}">
                                <textarea name="comentario" class="reply-txt tynemce-on"
    ...
    <div id="paginacao" class="posreflole">
        <ul class="pagination">
            {if $pages.previous}
                <li class="item">
                    <a class="linkpagination" title="Página Anterior"
                       href="{$this->url(["module"=>"default", "controller"=>"forum", "action"=>"detalheforum", "titulo"=>{$this->createslug($topico->DS_TOPICO_TOSO)}, "idforum"=>{$topico->NR_SEQ_TOPICO_TOSO}, "page"=>{$pages.next}], "detalheforum", TRUE)}"><</a>
                </li>
            {/if}
            {section name=page_loop start=$this->mensagens->current_page-1 loop=$this->mensagens->current_page+3 step=1}
                <li class="item">
                    <a class="linkpagination"
                       href="{$this->url(["module"=>"default", "controller"=>"forum", "action"=>"detalheforum", "titulo"=>{$this->createslug($topico->DS_TOPICO_TOSO)}, "idforum"=>{$topico->NR_SEQ_TOPICO_TOSO},"page"=>$smarty.section.page_loop.index+1], "detalheforum", TRUE)}">
            ...
            {if $pages.next}
                <li class="item">
                    <a class="linkpagination" title="Próxima Página"
                       href="{$this->url(["module"=>"default", "controller"=>"forum", "action"=>"detalheforum", "titulo"=>{$this->createslug($topico->DS_TOPICO_TOSO)}, "idforum"=>{$topico->NR_SEQ_TOPICO_TOSO}, "page"=>{$pages.next}], "detalheforum", TRUE)}">></a>
                </li>
            {/if}
        </ul>
