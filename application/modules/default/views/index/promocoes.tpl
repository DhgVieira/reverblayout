...
<section class="products">
    <h1 class="rvb-title">Reverb <span>Promoções</span></h1>
    ...
    <div class="rvb-column left">
                ...
                <a href="{$this->url(["titulo"=>{$this->createslug($slug)}, "idproduto"=>{$produto->NR_SEQ_PRODUTO_PRRC}], 'produto', TRUE)}" class="product-photo">
                    {if file_exists("arquivos/uploads/fotosprodutos/$foto_completa")}
                        ...
                        <span data-picture data-alt="{$produto->DS_PRODUTO_PRRC}" data-title="{$produto->DS_PRODUTO_PRRC}">
                            ...
                            <noscript>
                                <img src="{$this->Url(['tipo'=>"fotosprodutos", 'crop'=>1, 'largura'=>160, 'altura'=>185, 'imagem'=>$foto_completa],"imagem", TRUE)}" alt="Imagem não encontrada - Reverbcity">
                            </noscript>
                        </span>
                    {else}
                        <!-- Polyfill para imagens responsivas-->
       ...
       {if $contadores|count > 16}
            <ul class="pagination">
                {if $pages.previous}
                    <li class="item">
                        <a title="Página Anterior" href="{$this->url(["module"=>"default", "controller"=>"loja", "action"=>"{$acaoAtual}", "page"=>{$pages.previous}, "categoria"=>{$cat['NR_SEQ_CATEGPRO_PCRC']}, "tamanho"=>{$tamanho_url}, "genero"=>{$genero}, "cor"=>{$cor_url}, "tipo"=>{$tipo_url}, "valor"=>{$valor_url}], "{$acaoAtual}", TRUE)}">◄</a>
                    </li>
                {/if}

                     {section name=page_loop start=0 loop=$this->contadores->total_pages step=1}
                            <li class="item">
                                <a href="{$this->url(["module"=>"default","controller"=>"loja",  "action"=>"{$acaoAtual}", "page"=>$smarty.section.page_loop.index+1, "categoria"=>{$cat['NR_SEQ_CATEGPRO_PCRC']}, "tamanho"=>{$tamanho_url}, "genero"=>{$genero}, "cor"=>{$cor_url}, "tipo"=>{$tipo_url}, "valor"=>{$valor_url}], "{$acaoAtual}", TRUE)}"  {if $smarty.section.page_loop.index+1 == $this->contadores->current_page} class="active" {/if}>
                ...
                {if $pages.next}
                    <li class="item">
                        <a title="Página Anterior" href="{$this->url(["module"=>"default", "controller"=>"loja", "action"=>"{$acaoAtual}", "page"=>{$pages.next}, "categoria"=>{$cat['NR_SEQ_CATEGPRO_PCRC']}, "tamanho"=>{$tamanho_url}, "genero"=>{$genero}, "cor"=>{$cor_url}, "tipo"=>{$tipo_url}, "valor"=>{$valor_url}], "{$acaoAtual}", TRUE)}">►</a>
                    </li>
                {/if}
            </ul>
        {/if}
