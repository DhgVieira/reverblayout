                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="row posr fw lt">
                        <div class="reverb-fields reverb-field-1">
                            ...
                            <input  class="bs reverb-input-1" type="text" id="titulo" name="DS_DESCRICAO_BARC" value="{$dadosBanner->DS_DESCRICAO_BARC}">
                        </div>
                    </div>
                    <div class="row posr fw lt">
                        <div class="reverb-fields reverb-field-1">
                            <label class="reverb-label-1" for="subtitulo">Subtítulo</label>
                            <input  class="bs reverb-input-1" type="text" id="subtitulo" name="MSG_BANNER_BARC" value="{$dadosBanner->MSG_BANNER_BARC}">
                        </div>
                    </div>
                    <div class="row posr fw lt">
                        <div class="reverb-fields reverb-field-1">
                            ...
                            <div class="reverb-input-1 select-1">
                                <select class="select bs" id="localbanner" name="NR_SEQ_LOCAL_BARC">
                                    {foreach from=$dadosBannerslocal item=local}
                                        <option value="{$local->NR_SEQ_BANLOCAL_BLRC}" {if $local->NR_SEQ_BANLOCAL_BLRC == $dadosBanner->NR_SEQ_LOCAL_BARC}selected {assign var=localNome value=$local->DS_LOCAL_BLRC}{/if}>{$local->DS_LOCAL_BLRC}</option>
                                    {/foreach}
                                </select>
                                <span class="select-value">{$localNome}</span>
                            </div>
                        </div>
                    </div>
                    <div class="row posr fw lt">
                        <div class="reverb-fields reverb-field-1">
                            ...
                            <input  class="bs reverb-input-1" type="text" id="destino" name="DS_LINK_BARC" value="{$dadosBanner->DS_LINK_BARC}">
                        </div>
                    </div>
                    {*<div class="row posr fw lt">*}
                    ...
                    <div class="row posr fw lt" style="margin-top: 10px;">
                        <div class="fw lt">
                            {assign var=fotoCompleta value="{$dadosBanner->NR_SEQ_BANNER_BARC}.{$dadosBanner->DS_EXT_BARC}"}
                            <img src="{$this->Url(['tipo'=>"banners", 'crop'=>1, 'largura'=>0, 'altura'=>0, 'imagem'=>$fotoCompleta],"imagem", TRUE)}" alt=""/>
                        </div>
                    </div>
                    <div class="row posr fw lt">
                        <div class="reverb-fields reverb-field-2">
                            ...
                            <div class="load-more-images posr image-uploader-1">
                                ...
                                <input type="file" class="upload-images posa" name="imagem" style="left: 0;">
                            </div>
                        </div>
                    </div>
                    {*<div class="row posr fw lt">*}
